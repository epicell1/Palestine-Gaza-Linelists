---
title: "Intentional Fresh Tauma Linelist Analysis - Attar"
author: "MSF OCBA"
...
setwd("C:/Users/msfe-jerusalem-epi/OneDrive - MSF/1_Epidemiology/3_Projects/12_Gaza Linelists/INTENTIONAL FRESH TRAUMA LINELIST")
...
# Read the "Attar" sheet, skipping first 4 rows
data <- read_excel(
  file_path,
  sheet = "Attar",
  skip = 5,
...
# Filter for distribution center only
# %>% 
#   filter(cause == "Distribution center")
...
# Define the correct age group order (reversed for top-to-bottom display)
age_group_levels <- rev(c("Pediatric (<5)", "Pediatric (5-17)", "Adult (18-39)", "Middle-aged (40-59)", "Elderly (60+)", "Unknown"))
...
```{r exit_heatmap, fig.width=8.5, fig.height=2.5, echo=FALSE, warning=FALSE, message=FALSE}
...
heatmap_data_exit <- data %>%
  ...
  mutate(
    ...
    exit_status = factor(exit_status, levels = c("Not specified", "Discharged Against Medical Advice", "Transferred", "Discharged"))
  )
